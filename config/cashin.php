...
// if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($amount == 50) {
        ...
        $sqlstmnt = "INSERT INTO emoney (UserID, gcashnum, Amount, ReferenceNum, ConFee, EMoneyStatus) Values ('$id', '$gcashnum', '$amount', '$ref', '10', 'pending')";
        $res = $connection->query($sqlstmnt);

        // $sqlstmnt = "UPDATE userinfo SET uBalance = uBalance + '40' WHERE UserID = '$id'";
        // $res = $connection->query($sqlstmnt);
    } else if ($amount == 100) {
        $sqlstmnt = "INSERT INTO emoney (UserID, gcashnum, Amount, ReferenceNum, ConFee, EMoneyStatus) Values ('$id', '$gcashnum', '$amount', '$ref', '20', 'pending')";
        $res = $connection->query($sqlstmnt);

        // $sqlstmnt = "UPDATE userinfo SET uBalance = uBalance + '80' WHERE UserID = '$id'";
        // $res = $connection->query($sqlstmnt);
    } else if ($amount == 250) {
        $sqlstmnt = "INSERT INTO emoney (UserID, gcashnum, Amount, ReferenceNum, ConFee, EMoneyStatus) Values ('$id', '$gcashnum', '$amount', '$ref', '50', 'pending')";
        $res = $connection->query($sqlstmnt);

        // $sqlstmnt = "UPDATE userinfo SET uBalance = uBalance + '200' WHERE UserID = '$id'";
        // $res = $connection->query($sqlstmnt);
    } else if ($amount == 500) {
        $sqlstmnt = "INSERT INTO emoney (UserID, gcashnum, Amount, ReferenceNum, ConFee, EMoneyStatus) Values ('$id', '$gcashnum', '$amount', '$ref', '50', 'pending')";
        $res = $connection->query($sqlstmnt);
        // $sqlstmnt = "UPDATE userinfo SET uBalance = uBalance + '450' WHERE UserID = '$id'";
        // $res = $connection->query($sqlstmnt);
    }

    $_SESSION['bg'] =  "warning";
    $_SESSION['message'] = "Your Cash In is now pending for approval.";
